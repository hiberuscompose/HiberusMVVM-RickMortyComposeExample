package com.hiberus.template.core.common.svg
...
class Vector2SvgConverter(val colors: ResourceCollector) {
    ...
    private fun convert(doc: Document) {
        with(doc.documentElement) {
            ...
            appendAttribute("viewBox", "0 0 ${attributes["width"]} ${attributes["height"]}")
    ...
    private fun convertAaptAttributesElement(doc: Document, element: Element): Pair<String, String> {
        val svgAttributeName: String = when (val name = element.getAttribute("name")) {
            "android:fillColor" -> {
                "fill"
            }
            "android:strokeColor" -> {
        ...
        val firstChildElement = element.childNodes.iterable.filterIsInstance<Element>().firstOrNull() ?: throw Exception("Childless aapt:attr")
    ...
    private fun createLinearGradient(doc: Document, gradientElement: Element, id: String): Element {
        ...
        when (gradientElement.getAttributeNS(ANDROID_NS, "tileMode").lowercase(Locale.getDefault())) {
            "clamp" ->
        ...
        if (startColor.isNotEmpty() && endColor.isNotEmpty()) {
            stops = if (centerColor.isNotEmpty()) {
                listOf(
    ...
    private fun parseColor(color: String): Pair<String, Float?> {
        ...
        if (color.startsWith("@")) {
            ...
            val resourceHex = colors.getValue(name) ?: throw IllegalArgumentException("Color $name does not exists")
    ...
    private fun parseColorHex(colorHex: String): Pair<String, Float?> {
        val color: String = if (colorHex.length < 6) {
            colorHex.takeLast(3)
        } else {
            colorHex.takeLast(6)
        }
    ...
    private fun removeBlankNodes(doc: Document) {
        ...
        val xpathQuery = "//text()[normalize-space(.) = '']"
        val xpath: XPathExpression = XPathFactory.newInstance().newXPath().compile(xpathQuery)
    ...
    private fun Element.fixScale() {
        ...
        if (scaleX != null || scaleY != null) {
            val scaleX1 = scaleX ?: "1"
            val scaleY1 = scaleY ?: "1"

            appendAttribute("transform", "scale($scaleX1, $scaleY1)")
        }
    ...
    private fun Element.fixRotate() {
        ...
        val rotation = attributes.get(ANDROID_NS, "rotation") ?: return
